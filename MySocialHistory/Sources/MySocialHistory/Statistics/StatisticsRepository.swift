...
struct StatisticsRepository {
    ...
    func fetchOverviewStats() async throws -> OverviewStats {
        try await dbQueue.read { db in
            ...
            let friends   = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM friends WHERE status = 'current'") ?? 0
    ...
    func fetchFriendsPerYear() async throws -> [FriendYearCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT
                    strftime('%Y', timestamp, 'unixepoch') AS year,
                    SUM(CASE WHEN status = 'current' THEN 1 ELSE 0 END) AS added,
    ...
    func fetchRemovedFriends() async throws -> [RemovedFriend] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                ...
                """)
            return rows.map {
                RemovedFriend(name: $0["name"], timestamp: $0["timestamp"])
            }
        }
    }
    ...
    func fetchFriendCount() async throws -> Int {
        try await dbQueue.read { db in
            try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM friends WHERE status = 'current'") ?? 0
        }
    }

    func fetchRejectedRequests() async throws -> [RemovedFriend] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT name, timestamp
                FROM friends
    ...
    func fetchLoginsPerMonth() async throws -> [MonthCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT strftime('%Y-%m', timestamp, 'unixepoch') AS month, COUNT(*) AS cnt
                FROM logins
    ...
    func fetchLoginDevices() async throws -> [DeviceCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                ...
                ORDER BY cnt DESC
                """)
            return rows.map { DeviceCount(device: $0["device_type"], count: $0["cnt"]) }
        }
    }

    func fetchLoginCities(limit: Int = 15) async throws -> [CityCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                ...
                LIMIT ?
                """, arguments: [limit])
            return rows.map { CityCount(city: $0["city"], country: $0["country"], count: $0["cnt"]) }
        }
    }

    func fetchLoginCount() async throws -> Int {
        try await dbQueue.read { db in
            try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM logins") ?? 0
        }
    }

    func fetchLoginByHour() async throws -> [HourCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT
    ...
    func fetchSearchesPerMonth() async throws -> [MonthCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT strftime('%Y-%m', timestamp, 'unixepoch') AS month, COUNT(*) AS cnt
                FROM searches
    ...
    func fetchTopSearchQueries(limit: Int = 20) async throws -> [TopSearchQuery] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                ...
                LIMIT ?
                """, arguments: [limit])
            return rows.map { TopSearchQuery(query: $0["q"], count: $0["cnt"]) }
        }
    }

    func fetchSearchCount() async throws -> Int {
        try await dbQueue.read { db in
            try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM searches") ?? 0
        }
    }

    func fetchSearchByHour() async throws -> [HourCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT
                    CAST(strftime('%H', timestamp, 'unixepoch') AS INTEGER) AS hour,
    ...
    func fetchMsgReactionsPerMonth() async throws -> [MsgReactionMonthCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT
                    strftime('%Y-%m', m.timestamp_ms / 1000, 'unixepoch') AS month,
                    COUNT(*) AS cnt
                FROM reactions r
                ...
                ORDER BY month ASC
                """)
            return rows.map { MsgReactionMonthCount(month: $0["month"], count: $0["cnt"]) }
        }
    }

    func fetchMostEmotionalThreads(limit: Int = 20, userName: String, isGroup: Bool) async throws -> [EmotionalThread] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT
                    t.id AS thread_id,
                ...
                WHERE t.message_count >= 100
                  AND r.actor_name = ?
                  AND t.is_group_chat = ?
                GROUP BY t.id
                ORDER BY (CAST(COUNT(r.id) AS REAL) / t.message_count) DESC
                LIMIT ?
                """, arguments: [userName, isGroup ? 1 : 0, limit])
            return rows.map {
                EmotionalThread(
                    ...
                    reactionCount: $0["reaction_count"],
                    isGroupChat: ($0["is_group_chat"] as Int) != 0
                )
            }
        }
    }

    func fetchMsgReactionCount() async throws -> Int {
        try await dbQueue.read { db in
            try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM reactions") ?? 0
        }
    }

    func fetchTopMsgReactionEmojis(limit: Int = 10) async throws -> [MsgReactionEmojiCount] {
        try await dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                ...
                """, arguments: [limit])
            return rows.map {
                let raw: String = $0["reaction_emoji"]
                // Normalize heart: ❤ (U+2764) → ❤️ (with variant selector)
                let emoji = raw == "\u{2764}" ? "❤️" : raw
                return MsgReactionEmojiCount(emoji: emoji, count: $0["cnt"])
            }
        }
    }
