...
final class ImportPipeline {
    ...
    private func runImport(exportRoot: URL) async throws {
        ...
        // Phase 7: Friends (from connections/friends/ alongside activity root)
        progress.phase = .friends
        let friendImporter = FriendImporter(dbQueue: db.dbQueue)
        for activityRoot in activityRoots {
            let exportParent = activityRoot.deletingLastPathComponent()
            let friendsPath = exportParent
                .appendingPathComponent("connections")
                .appendingPathComponent("friends")
                .appendingPathComponent("your_friends.json")
            if FileManager.default.fileExists(atPath: friendsPath.path) {
                progress.totalFriends += try await friendImporter.importAll(exportParent: exportParent)
                break
            }
        }
        ...
        // Phase 8: Login activity (from security_and_login_information/ alongside activity root)
        progress.phase = .logins
        let loginImporter = LoginImporter(dbQueue: db.dbQueue)
        for activityRoot in activityRoots {
            let exportParent = activityRoot.deletingLastPathComponent()
            let loginsPath = exportParent
                .appendingPathComponent("security_and_login_information")
                .appendingPathComponent("account_activity.json")
            if FileManager.default.fileExists(atPath: loginsPath.path) {
                progress.totalLogins += try await loginImporter.importAll(exportParent: exportParent)
                break
            }
        }
        ...
        // Phase 9: Search history (from logged_information/search/ alongside activity root)
        progress.phase = .searches
        let searchImporter = SearchImporter(dbQueue: db.dbQueue)
        for activityRoot in activityRoots {
            let exportParent = activityRoot.deletingLastPathComponent()
            let searchPath = exportParent
            ...
            if FileManager.default.fileExists(atPath: searchPath.path) {
                progress.totalSearches += try await searchImporter.importAll(exportParent: exportParent)
                break
            }
        }
        ...
        // Phase 10: Notifications (from logged_information/notifications/ alongside activity root)
        progress.phase = .notifications
        let notificationImporter = NotificationImporter(dbQueue: db.dbQueue)
        for activityRoot in activityRoots {
            let exportParent = activityRoot.deletingLastPathComponent()
            let notifPath = exportParent
                .appendingPathComponent("logged_information")
                .appendingPathComponent("notifications")
                .appendingPathComponent("notifications.json")
            if FileManager.default.fileExists(atPath: notifPath.path) {
                progress.totalNotifications += try await notificationImporter.importAll(exportParent: exportParent)
                break
            }
        }
        ...
        // Phase 11: Visit history (from logged_information/interactions/ alongside activity root)
        progress.phase = .visits
        let visitImporter = VisitImporter(dbQueue: db.dbQueue)
        for activityRoot in activityRoots {
            let exportParent = activityRoot.deletingLastPathComponent()
            let visitPath = exportParent
                .appendingPathComponent("logged_information")
                .appendingPathComponent("interactions")
                .appendingPathComponent("recently_visited.json")
            if FileManager.default.fileExists(atPath: visitPath.path) {
                progress.totalVisits += try await visitImporter.importAll(exportParent: exportParent)
                break
            }
        }
        ...
        // Phase 12: Profile (import from first root that has personal_information/)
