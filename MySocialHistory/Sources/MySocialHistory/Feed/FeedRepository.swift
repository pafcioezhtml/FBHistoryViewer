...
struct FeedRepository {
    ...
    func fetchPage(
        ...
        filter: FeedFilter,
        searchText: String = "",
        postSubFilter: PostSubFilter = .own
    ) async throws -> [FeedItem] {
        switch filter {
        case .all:      return try await fetchMixed(beforeTimestamp: beforeTimestamp, searchText: searchText)
        case .posts:    return try await fetchPosts(beforeTimestamp: beforeTimestamp, searchText: searchText, postSubFilter: postSubFilter)
    ...
    func fetchPostSubCounts() async throws -> [String: Int] {
        try await dbQueue.read { db in
            let total = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM posts") ?? 0
            let group = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM posts WHERE source = 'group'") ?? 0
            let wall = try Int.fetchOne(db, sql: "SELECT COUNT(*) FROM posts WHERE source = 'timeline' AND (title LIKE '%wrote on%profile%' OR title LIKE '%shared%to%timeline%')") ?? 0
            let own = total - group - wall
            return ["own": own, "wall": wall, "group": group]
        }
    }

    // MARK: - Filtered single-table fetches

    private func fetchPosts(beforeTimestamp: Int64?, searchText: String, postSubFilter: PostSubFilter = .own) async throws -> [FeedItem] {
        try await dbQueue.read { db in
            ...
            switch postSubFilter {
            case .own:
                conditions.append("source = 'timeline'")
                conditions.append("(title IS NULL OR (title NOT LIKE '%wrote on%profile%' AND title NOT LIKE '%shared%to%timeline%'))")
            case .wall:
                conditions.append("source = 'timeline'")
                conditions.append("(title LIKE '%wrote on%profile%' OR title LIKE '%shared%to%timeline%')")
            case .group:
                conditions.append("source = 'group'")
            }
