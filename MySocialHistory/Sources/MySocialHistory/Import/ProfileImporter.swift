...
actor ProfileImporter {
    ...
    func importProfile(activityRoot: URL) async throws {
        ...
        // Collect profile update history (all title entries)
        let updateRows = collectProfileUpdates(exportParent: exportParent)
        ...
        try await dbQueue.write { db in
            ...
            try db.execute(sql: "DELETE FROM profile_updates")
            for u in updateRows {
                try db.execute(
                    sql: "INSERT INTO profile_updates (timestamp, title, detail, image_uri) VALUES (?, ?, ?, ?)",
                    arguments: [u.timestamp, u.title, u.detail, u.imageURI]
                )
            }
        }
    }
    ...
    // MARK: - Profile update history

    private func collectProfileUpdates(exportParent: URL) -> [(timestamp: Int64, title: String, detail: String?, imageURI: String?)] {
        let updateHistoryURL = exportParent
            .appendingPathComponent("personal_information")
            .appendingPathComponent("profile_information")
            .appendingPathComponent("profile_update_history.json")

        guard let data    = try? Data(contentsOf: updateHistoryURL),
              let history = try? JSONDecoder().decode(RawProfileUpdateHistory.self, from: data)
        else { return [] }

        return (history.profile_updates_v2 ?? []).compactMap { update in
            guard let ts = update.timestamp,
                  let title = update.title, !title.isEmpty else { return nil }
            // Filter out noise: "added * to his/her profile/favorite *"
            let lower = title.lowercased()
            if lower.contains("added") && (lower.contains("to his profile") || lower.contains("to her profile")) { return nil }
            if lower.contains("added") && lower.contains("to his favorite") { return nil }
            if lower.contains("added") && lower.contains("to her favorite") { return nil }
            // Also filter "added X as his/her hometown" style
            if lower.contains("added") && lower.contains("as his") { return nil }
            if lower.contains("added") && lower.contains("as her") { return nil }

            // Extract image URI from attachments (profile picture updates)
            let imageURI = update.attachments?.first?.data?.first?.media?.uri

            // Extract detail text: from attachments[].data[].text (bio) or data[].text (website, screen name)
            let detail: String?
            if let attachText = update.attachments?.first?.data?.first(where: { $0.media == nil })?.text {
                detail = attachText
            } else if let topText = update.data?.compactMap({ $0.text }).first {
                detail = topText
            } else {
                detail = nil
            }

            return (timestamp: ts, title: title.fixedFacebookEncoding, detail: detail?.fixedFacebookEncoding, imageURI: imageURI)
        }
    }
...
private struct RawProfileUpdate: Decodable {
    let timestamp: Int64?
    let title: String?
    let data: [RawProfileUpdateData]?
    let attachments: [RawUpdateAttachment]?
}

private struct RawProfileUpdateData: Decodable {
    let text: String?
}
